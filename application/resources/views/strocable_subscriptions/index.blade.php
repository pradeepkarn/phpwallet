  ...
  <div class="col-lg-9 col-md-12">
    <div class="row">
      <div class="col"  id="#sendMoney">
        ...
        <div class="card">
          <div class="header">
            <h2><strong>{{__('Strowallet Cable')}}</strong></h2>
          </div>
          <div class="body">
            <div class="card">
              <div class="card-body">
                <div class="alert_message alert" style="display: none;"></div>
                  <form action="{{route('cable_merchant_verify',app()->getLocale())}}" method="POST">
                    @csrf
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <select name="service_id" class="form-control service_id" required="">
                                    ...
                                    <option data-url="{{route('strogetCablePlan',[app()->getLocale(),'dstv'])}}" <?php echo (isset($service_id) && $service_id == 'dstv') ? 'selected':''; ?> value="dstv">
                                      DSTV
                                    </option>
                                    <option data-url="{{route('strogetCablePlan',[app()->getLocale(),'gotv'])}}" <?php echo (isset($service_id) && $service_id == 'gotv') ? 'selected':''; ?> value="gotv">
                                      GOTV
                                    </option>
                                    <option data-url="{{route('strogetCablePlan',[app()->getLocale(),'startimes'])}}" <?php echo (isset($service_id) && $service_id == 'startimes') ? 'selected':''; ?> value="startimes">
                                      STARTIMES
                                    </option>
                                    <option data-url="{{route('strogetCablePlan',[app()->getLocale(),'showmax'])}}" <?php echo (isset($service_id) && $service_id == 'showmax') ? 'selected':''; ?> value="showmax">
                                      ShowMax
                                    </option>
                                </select>
                            </div>
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  $('body').on('click','#submit',function(event){
    event.preventDefault();
    var action="{{route('cable_merchant_verify',app()->getLocale())}}";
    ...
    $.ajax({
      ...
      success: function( response ) {
        ...
        if(result.success == 1)
        {
          ...
          window.location.href = "{{route('cablePreview',app()->getLocale())}}";
        }
        else
        {
          $('body').find('.alert_message').show();
